...
@(define demo (make-base-eval #:lang 'flit))
@(define demo2 (make-base-eval #:lang 'flit))
...
The flit language syntactically resembles the
...
The body of a @schememodname[flit] module is a sequence of
definitions, expressions and type declarations. The module implicitly exports all
top-level definitions. When a @racketmodname[flit] module is
imported into a module that does not use @racketmodname[flit],
...
placing a @litchar{'} in from of any @racketmodname[flit]
...
text. Various @racketmodname[flit] values, including symbols, numbers,
and lists, can be coerced to and from S-expression form.

The representation of an S-expression always reuses some other
@racketmodname[flit] value, so conversion to and from an S-expression
...
@racketmodname[flit] program. They can be used in untyped contexts to
coerce a @racketmodname[flit] @tech{S-expression} to a plain Racket
...
These functions have no type, so they cannot be used in a
@racketmodname[flit] program. They can be used in untyped contexts to
coerce a @racketmodname[flit] @tech{tuple} to an immutable vector
and vice-versa.}
...
@#,racketmodname[flit]] to disable type checking. The syntax of a
@racketmodname[flit] module is the same with and without
@racket[#:untyped], but types are ignored when @racket[#:untyped] is
specified. An untyped flit module can interoperate with a typed flit
...
@#,racketmodname[flit]] to switch evaluation to lazy mode. The syntax
...
ultimately). A lazy flit module will not interoperate well with an
eager module.

Use @racket[#:fuel _amount] immediately after @racket[@#,hash-lang[]
@#,racketmodname[flit]] to specify how much effort should be spent
